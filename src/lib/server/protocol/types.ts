...
export interface HealthCheckOutput {
	ok: boolean;
	version?: string;
	// Optional capability advertisement. Endpoints list the identifiers of any
	// optional protocol features they support here (see PROTOCOL_FEATURES).
	// Sidekick treats a missing/empty array as "supports nothing optional", so
	// older endpoints that predate capability negotiation continue to work.
	features?: string[];
}
...
// ---------------------------------------------------------------------------
// Identifiers an endpoint may advertise in HealthCheckOutput.features to opt
// into optional functionality. Sidekick only surfaces a capability's UI when
// the connected endpoint advertises it.

export const PROTOCOL_FEATURES = {
	// The endpoint can serve the full project catalogue (any ship status) via
	// FETCH_PROJECTS, not just the pending review queue. Enables the Projects tab.
	PROJECTS: 'projects'
} as const;
...
export type SidekickResponse<T extends SidekickRequest['action']> = ActionOutputMap[T];
